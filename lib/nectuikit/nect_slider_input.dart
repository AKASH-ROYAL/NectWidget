...
class NectSliderInput extends StatefulWidget {
  double value;
  ...
  NectSliderInput({
    ...
    this.max = 100,
    this.vertical = false,
    this.primaryColor = NectColor.Blue3,
    this.secondaryColor = NectColor.Gray11,
    this.disabled = false,
    this.controlColor = Colors.white,
    this.divisions = 100,
    this.showLabel = true,
    this.valueIndicatorColor = NectColor.Gray3,
    this.valueIndicatorTextStyle = NectTextStyle.NectCaption,
    this.valueIndicatorTextColor = Colors.white
  });

  @override
  _NectSliderInputState createState() => _NectSliderInputState();

}

class _NectSliderInputState extends State<NectSliderInput> {
  _NectSliderInputState();
        ...
        child: Opacity(
          ...
          child: SliderTheme(
              data: SliderTheme.of(context).copyWith(
                  ...
                  trackShape: NectSliderTrackShape(active: active),
                  trackHeight: 4,
                  overlayShape: const RoundSliderOverlayShape(overlayRadius: 1),
                  overlayColor: widget.primaryColor,
                  thumbShape: NectSliderThumbShape(
                      thumbRadius: 28, active: active),
                  ...
                  valueIndicatorTextStyle: widget.valueIndicatorTextStyle.apply(
                      color: widget.valueIndicatorTextColor),
                  valueIndicatorShape: widget.vertical
                      ? const NectSliderValueIndicatorShape()
...
class NectSliderTrackShape extends RoundedRectSliderTrackShape {
  NectSliderTrackShape({required this.active});

  bool active;

  void ppaint(PaintingContext context,
      Offset offset, {
    ...
    context.canvas.drawRRect(
      RRect.fromLTRBAndCorners(
        trackRect.left,
        (textDirection == TextDirection.ltr) ? trackRect.top -
            ((active ? additionalActiveTrackHeight : 0) / 2) : trackRect.top,
        thumbCenter.dx,
        (textDirection == TextDirection.ltr) ? trackRect.bottom +
            ((active ? additionalActiveTrackHeight : 0) / 2) : trackRect.bottom,
    ...
    context.canvas.drawRRect(
      RRect.fromLTRBAndCorners(
        thumbCenter.dx,
        (textDirection == TextDirection.rtl) ? trackRect.top -
            ((active ? additionalActiveTrackHeight : 0) / 2) : trackRect.top,
        trackRect.right,
        (textDirection == TextDirection.rtl) ? trackRect.bottom +
            ((active ? additionalActiveTrackHeight : 0) / 2) : trackRect.bottom,
...
class NectSliderThumbShape extends SliderComponentShape {
  final double thumbRadius;
  bool active;

  NectSliderThumbShape({
  ...
  void paint(PaintingContext context, Offset center,
    ...
    final borderPaint = Paint()
      ..color = sliderTheme.activeTrackColor != null ? sliderTheme
          .activeTrackColor! : NectColor.Blue3
      ..strokeWidth = 2
      ..style = PaintingStyle.stroke;

    final linePaint = Paint()
      ..color = sliderTheme.inactiveTrackColor != null ? sliderTheme
          .inactiveTrackColor! : NectColor.Gray11
      ..style = PaintingStyle.fill;
    ...
    Rect shadowRect = Rect.fromCenter(center: center.translate(0, -4),
        width: thumbRadius - 1,
    ...
    canvas.drawShadow(Path()
      ..addRRect(shadowRRect), NectColor.Gray.withOpacity(0.25), 8, false);
    ...
    Rect lineRectMid = Rect.fromCenter(
        center: center, width: 1, height: thumbRadius - 2 - 10);
    ...
    Rect lineRectLeft = Rect.fromCenter(center: center.translate(-4.5, 0),
        width: 1,
    ...
    Rect lineRectRight = Rect.fromCenter(center: center.translate(4.5, 0),
        width: 1,
        height: thumbRadius - 2 - 10);
...
class NectSliderValueIndicatorShape extends SliderComponentShape {
  /// Create a slider value indicator that resembles a rectangular tooltip.
  const NectSliderValueIndicatorShape();

  static const _NectSliderValueShapePathPainter _pathPainter = _NectSliderValueShapePathPainter();
...
class _NectSliderValueShapePathPainter {
  const _NectSliderValueShapePathPainter();
  ...
  Size getPreferredSize(TextPainter labelPainter,
      double textScaleFactor,) {
    return Size(
  ...
  double getHorizontalShift({
    ...
    final double rectangleWidth = _upperRectangleWidth(
        labelPainter, scale, textScaleFactor);
    ...
    final double overflowLeft = math.max(
        0, rectangleWidth / 2 - globalCenter.dx + edgePadding);
    final double overflowRight = math.max(0, rectangleWidth / 2 -
  ...
  double _upperRectangleWidth(TextPainter labelPainter, double scale,
      double textScaleFactor) {
    final double unscaledWidth = math.max(
        _minLabelWidth * textScaleFactor, labelPainter.width) +
        _labelPadding * 2;
  ...
  void paint({
    ...
    final Path trianglePath = Path()
      ..lineTo(-_triangleHeight, -_triangleHeight)..lineTo(
          _triangleHeight, -_triangleHeight)
      ..close();
